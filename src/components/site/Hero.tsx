      ...
      <div className="container-luxury relative z-10 pt-36 pb-24">
        <div className="max-w-4xl">
          ...
          <h1 className="font-serif text-white text-5xl md:text-7xl lg:text-[5.25rem] leading-[1.02] mb-8 tracking-display font-medium">
            {t.hero.title1}<span className="italic text-gold">{t.hero.titleAccent}</span>{t.hero.title2}
          </h1>
          <p className="text-white/85 text-lg md:text-xl max-w-2xl mb-12 leading-relaxed font-light">
            {t.hero.sub}
          </p>
          <div className="flex flex-col sm:flex-row gap-4">
            <a href="#lead-capture" className="inline-flex items-center justify-center bg-gold text-ink font-semibold px-9 py-4 rounded-sm hover:opacity-90 transition-smooth shadow-luxury">
              {t.hero.ctaPrimary}
            </a>
            <a href="#evaluation" className="inline-flex items-center justify-center border border-white/40 text-white font-semibold px-9 py-4 rounded-sm hover:bg-white hover:text-ink transition-smooth">
              {t.hero.ctaSecondary}
            </a>
          </div>
        </div>
